...
let findByApiToken = function (apiToken) {
    return new Promise((resolve, reject) => {
        let whereStr = { apiToken: apiToken };
...
//将一个用户信息更新插入到数据库表格中
let upsertApiToken = async function (result) {
    ...
    return new Promise((resolve, reject) => {
        let whereStr = { apiToken: result.apiToken }
        let updateStr = {
...
module.exports = () => {
    return async function (ctx, next) {
        ...
        //每次请求获得一个token 
        let apiToken = ctx.request.body.apiToken;
        ...
        if (result.length === 0) {
            ...
            //存入数据库
            let res = await upsertApiToken(result);
        }
        //本地数据库有信息
        else {
