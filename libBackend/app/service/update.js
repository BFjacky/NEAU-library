    ...
    class updateService extends app.Service {
        async updateGrxx(stuId, pswd, name) {
            ...
            */
            ...
            let stu = await tryUsefulCookie(stuId, pswd, name);
            //通过已激活的cookie获取个人信息的html页面
            let htmlData = await getgrxxHtml(stu.cookie);
            //将个人信息的html页面爬取下来
            let res = getgrxxFromHtml(htmlData);
            //将得到的数据填入数据库
        ...
        async updateHisBooks(stuId, pswd, name) {
            ...
            */
            try {
                ...
                let htmlDatas = await getHistoryBookHtml(stu.cookie);
                //将借阅历史的html页面爬取下来
                let datas = [];
        ...
        async updateNowBorrow(stuId, pswd, name) {
            ...
            */
            try {
                ...
                let stu = await tryUsefulCookie(stuId, pswd, name);
                //通过已激活的cookie获取借阅历史的html页面
                let htmlDatas = await getNowBorrowHtml(stu.cookie);
                //将借阅历史的html页面爬取下来
                let datas = [];
                for (let i = 1; i < htmlDatas.length; i++) {
